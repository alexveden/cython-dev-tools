...
DEBUG_TRACE = int(os.getenv('CYGDB_DEBUG_TRACE', 0))
def TRACE(message, level=0):
    if DEBUG_TRACE > level:
        print(message)
...
class ExecutionControlCommandBase(gdb.Command):
    ...
    def install_breakpoints(self):
        ...
        for location in all_locations:
            ...
            bp_re = re.search(r'Breakpoint (\d+)', result)
            TRACE(f'step: install_breakpoints-> {location} ({result})', 4)
            if bp_re:
                TRACE(f'step: install_breakpoints-> {location} #bp: {bp_re.group(1)}', 4)
                yield bp_re.group(1)

    def delete_breakpoints(self, breakpoint_list):
        for bp in breakpoint_list:
            TRACE(f'step: delete_breakpoints-> {bp}', 4)
    ...
    def _finish_frame(self):
        ...
        while True:
            ...
            try:
                ...
                TRACE('_finish_frame: gdb.selected_frame() runtime error')
                break
            ...
            if hitbp or is_relevant or self.stopped():
                TRACE(f'_finish_frame: hitbp={hitbp}, is_relevant={is_relevant}, self.stopped()={self.stopped()}', level=1)
                break
    ...
    def step(self, stepinto, stepover_command='next'):
        ...
        newframe = beginframe
        TRACE(f'step: {newframe}', 2)

        while True:
            if self.lang_info.is_relevant_function(newframe):
                gdb.flush()
                TRACE(f'step:  step_over >')
                #result = gdb.execute(stepover_command, to_string=True)
                result = gdb.execute(stepover_command, to_string=True)
                TRACE(f'step: relevant ```{result}```', 2)
            else:
                TRACE(f'step:  next finish >')
                result = self._finish_frame()
                TRACE(f'step: next result ```{result}```', 2)

            if self.stopped():
                TRACE('step: self.stopped()')
                break
            ...
            if m:
                if is_relevant_function and m.group(1) in breakpoint_list:
                    ...
                    TRACE('step: Breakpoint line found')
                    break
            ...
            if newframe != beginframe:
                ...
                if not stepinto:
                    ...
                    TRACE(f'step: stack_depth: {newdepth}')
                ...
                if lineno and lineno != beginline:
                    TRACE(f'step: stack_depth: line no diff lineno={lineno} != beginline={beginline}')
                    break
